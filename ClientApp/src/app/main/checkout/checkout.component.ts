...
export class CheckoutComponent implements OnInit {
  ...
  constructor(
    ...
    this.http
      .get(environment.URL_API + 'carts/getcartbyuserid/' + this.id_user, {})
      .subscribe((res) => {
        this.list_item = res;
        ...
        for (let i = 0; i < this.list_item.length; i++) {
          this.tongtien =
            this.tongtien +
            this.list_item[i].productDetail.price * this.list_item[i].quantity;
    ...
    this.check = null;
    this.http
      .get(environment.URL_API + 'users/getUserAddress/' + this.id_user, {
        responseType: 'text',
      })
      .subscribe((res) => {
  ...
  deleteSanPham(item): void {
    ...
    this.http
      .post(environment.URL_API + 'Carts/delete', delproduct)
      .subscribe((res) => {
        Swal.fire('Xoá sản phẩm khỏi giỏ hàng thành công .', '', 'success');
        this.http
          .get(environment.URL_API + 'Carts/getcartbyuserid/' + clicks, {})
          .subscribe((res) => {
            this.list_item = res;
            this.tongtien = 0;
            for (let i = 0; i < this.list_item.length; i++) {
              this.tongtien =
                this.tongtien +
                this.list_item[i].productDetail.price *
                  this.list_item[i].quantity;
              this.tongThanhToan = this.tongtien;
            }
  ...
  maGiamGia(): void {
    if (localStorage.getItem('products') === null) {
      ...
      this.check = this.list_MGG.filter(
        (d) => d.couponName == this.couponName
      )[0];
      //kiểm tra xem mã giảm giá được sử dụng chưa
      this.check_sudung = this.list_MGGSD.filter(
        (d) => d == this.couponName
      )[0];
  ...
  show(): void {
    if (localStorage.getItem('products') === null) {
      ...
      this.http
        .post<ApiResponse>(environment.URL_API + 'orders', val)
        .subscribe(
          (res: ApiResponse) => {
            console.log(res);
            if (res.urlVNPay) {
              ...
              Swal.fire(
                'Đặt hàng thành công, chuyển sang bước thanh toán.',
                '',
                'success'
              ).then(() => {
  ...
  onSubmit(): void {
    if (localStorage.getItem('products') === null) {
      ...
      this.http.post(environment.URL_API + 'orders', val).subscribe((res) => {
  ...
  ChangeSoLuong(cartID, i) {
    ...
    this.http
      .post(environment.URL_API + 'Carts/update/', val)
      .subscribe((res) => {
        this.list_item = res;
        this.tongtien = 0;
        for (let i = 0; i < this.list_item.length; i++) {
  ...
  updateCongSanPham(cartID, soLuong) {
    ...
    this.http
      .put(environment.URL_API + 'Carts/update/', val)
      .subscribe((res) => {
        this.list_item = res;
        this.tongtien = 0;
        for (let i = 0; i < this.list_item.length; i++) {
