...
export class BillDetailsComponent implements OnInit {
  ...
  constructor(public http: HttpClient, public route: ActivatedRoute) {
    this.route.params.subscribe((params) => {
      ...
      this.http
        .get(environment.URL_API + 'orders/' + this.id_bill, {})
        .subscribe((res) => {
          this.bill = res;
        });
      this.http
        .get(environment.URL_API + 'orderdetails/' + this.id_bill, {})
        .subscribe((res) => {
          ...
          for (let i = 0; i < this.bill_details.length; i++) {
            this.tongtien =
              this.tongtien +
              this.bill_details[i].price * this.bill_details[i].quantity;
          }
        });
    });
  }
  Huy() {
    ...
    this.http
      .put(environment.URL_API + 'orders/updatestatus/' + this.id_bill, val)
      .subscribe((res) => {
